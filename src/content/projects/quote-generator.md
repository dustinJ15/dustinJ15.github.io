...
metrics:
  ...
  - value: 1.5 days
    label: of manual assembly removed
---
...
write does three things at once: it claims the quote, it flips the business lead's dashboard from
"sent to proposal specialist" to "drafting in progress", and it closes the lead's ability to keep
editing the intake underneath them. The claim and the signal are the same event, so they cannot
disagree. The check runs inside the transaction, so a lead who saves at the same instant loses
cleanly rather than silently winning.
...
otherwise modify it refuses. The check runs on the way in, so the caller gets a clear error, and
again inside the transaction, which is the one that actually holds. After that, amendments go
through change orders, which baseline against exactly what was signed rather than against
whatever the quote looks like now.
...
checking, security scanning. CI runs that exact command, so "green on my machine" and "green
...
them being filed again. Treating "we have not actually confirmed this" as a fact worth writing
down is the habit I would bring to a team, more than any particular framework.
